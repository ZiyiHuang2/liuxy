...
The project moved through six main modeling ideas.
...
6. **Retained Unknown VOC audit** reconstructed 335 filtered Unknown features and tested direct concatenation, joint filtering, Unknown-only modeling and low-weight probability fusion. Direct retention degraded discovery F1. A 5% probability fusion improved the paired 32-seed mean by only 0.00096 with a confidence interval crossing zero, so the known-only model remained frozen.
